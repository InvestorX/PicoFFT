...
void fft_analyzer_init(void) {
    ...
    // Set ADC clock divider for reasonable sampling
    // Using direct adc_read() calls, so no need for specific timing here
    adc_set_clkdiv(390.0f);  // Maintain 128kHz rate for consistency
...
void fft_process_data(void) {
    // Sample ADC data directly using adc_read()
    for (int i = 0; i < FFT_SIZE; i++) {
        g_fft_analyzer.adc_buffer[i] = adc_read();
        
        // Small delay to maintain approximately 128kHz sampling rate
        // 128kHz = 7.8µs per sample, but adc_read() takes some time
        sleep_us(1);  // Adjust as needed for timing
    }
